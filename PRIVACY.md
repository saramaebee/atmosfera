...
atmosfera is a climate-charts bot. It does not track members: no message content and no per-user activity data are stored. The privacy-relevant surfaces are the city-lookup flow and the admin tooling (permissions + audit log).
...
The bot keeps a record of admin actions (who toggled what, when): actor user ID, event type (e.g. `permission.grant`), the affected subject, before/after metadata, and a timestamp. Regular user activity is not logged. Server admins can view it via `/permissions audit`; retention is indefinite (it's an administrative record).
